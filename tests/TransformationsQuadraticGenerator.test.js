const TransformationsQuadraticGenerator = require("../src/engine/generators/functions/topics/quadratic/TransformationsQuadraticGenerator");

console.log("Running test for TransformationsQuadraticGenerator...");

function runTest() {
  try {
    const generator = new TransformationsQuadraticGenerator("hard");
    ...
    for (const method of methods) {
      ...
      for (let i = 0; i < iterations; i++) {
        ...
        if (method === "generateShiftParabolaProblem") {
          signature = `${qText}|${vars?.p}|${vars?.q}|${vars?.a}`;
        } else if (method === "generateInequalityGraphProblem") {
          signature = `${qText}|${vars?.x1}|${vars?.x2}|${vars?.sign}`;
        }
