const GeometricOptimizationGenerator = require("../src/engine/generators/functions/topics/optimization/GeometricOptimizationGenerator");

console.log("Running test for GeometricOptimizationGenerator...");

function runTest() {
  try {
    const generator = new GeometricOptimizationGenerator("hard");
    const iterations = 1000;

    const methods = ["generateFencingProblem", "generateCuboidProblem", "generateTrapezoidProblem"];
    ...
    for (const method of methods) {
      ...
      for (let i = 0; i < iterations; i++) {
        ...
        // Include variables in signature
        const vars = problem.content.variables;
        const signature = `${qText}|${JSON.stringify(vars)}`;
      ...
      // 95%+ uniqueness for these generators
