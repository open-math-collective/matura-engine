const ExponentialFunctionGenerator = require("../src/engine/generators/functions/topics/general/ExponentialFunctionGenerator");

console.log("Running test for ExponentialFunctionGenerator...");

function runTest() {
  try {
    const generator = new ExponentialFunctionGenerator("hard");
    const iterations = 1000;

    const methods = ["generateExponentialParam"];
    ...
    for (const method of methods) {
      ...
      if (uniqueCount < iterations * 0.9) {
        console.error(
          `FAIL: Too many collisions in ${method}. Expected >= 900 unique, got ${uniqueCount}.`,
        );
        process.exit(1);
      }
    }
