const PointsAndSegmentsGenerator = require("../src/engine/generators/geometry/topics/analytic/PointsAndSegmentsGenerator");

console.log("Running test for PointsAndSegmentsGenerator...");

function runTest() {
  try {
    const generator = new PointsAndSegmentsGenerator("hard");
    ...
    for (const method of methods) {
      ...
      for (let i = 0; i < iterations; i++) {
        ...
        if (method === "generateMidpointProblem") {
          signature = `${qText}|${vars?.A?.x}|${vars?.A?.y}|${vars?.B?.x}|${vars?.B?.y}`;
        } else if (method === "generateMissingEndpoint") {
          signature = `${qText}|${vars?.A?.x}|${vars?.A?.y}|${vars?.S?.x}|${vars?.S?.y}`;
        } else if (method === "generateDistanceUnknownCoord") {
          signature = `${qText}|${vars?.m}|${vars?.d}`;
        } else if (method === "generatePointSymmetry") {
          signature = `${qText}|${vars?.P?.x}|${vars?.P?.y}|${vars?.type}`;
        } else if (method === "generateCollinearPoints") {
          signature = `${qText}|${vars?.m_sol}`;
        }
