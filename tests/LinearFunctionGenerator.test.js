const LinearFunctionGenerator = require("../src/engine/generators/functions/topics/general/LinearFunctionGenerator");

console.log("Running test for LinearFunctionGenerator...");

function runTest() {
  try {
    const generator = new LinearFunctionGenerator("hard");
    ...
    for (const method of methods) {
      ...
      for (let i = 0; i < iterations; i++) {
        ...
        let signature = qText;
        if (method === "generateLinearRoot") {
          signature = `${qText}|${problem.content.variables?.root}`;
        } else if (method === "generateLinearMonotonicityParam") {
          signature = `${qText}|${problem.content.variables?.coeffM}|${problem.content.variables?.validConst}|${problem.content.variables?.type}`;
        }
        seen.add(signature);
        uniqueCount++;
      }
      ...
      let threshold = 0.98;
      if (method === "generateLinearRoot") threshold = 0.96;
      if (method === "generateLinearGraphAnalysis") threshold = 0.005;
      if (method === "generateLinearProperties") threshold = 0.005;
      if (method === "generateLinearMonotonicityParam") threshold = 0.94;
