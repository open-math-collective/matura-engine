const TransformationsGenerator = require("../src/engine/generators/functions/topics/general/TransformationsGenerator");

console.log("Running test for TransformationsGenerator...");

function runTest() {
  try {
    const generator = new TransformationsGenerator("hard");
    const iterations = 1000;

    const methods = ["generateSymmetryTransform", "generateFunctionShift"];
    ...
    for (const method of methods) {
      ...
      for (let i = 0; i < iterations; i++) {
        ...
        const signature =
          method === "generateSymmetryTransform"
            ? `${qText}|${vars?.type}|${vars?.baseFunc}`
            : `${qText}|${vars?.p}|${vars?.q}`;
