const ShapesCoordsGenerator = require("../src/engine/generators/geometry/topics/analytic/ShapesCoordsGenerator");

console.log("Running test for ShapesCoordsGenerator...");

function runTest() {
  try {
    const generator = new ShapesCoordsGenerator("hard");
    ...
    for (const method of methods) {
      ...
      for (let i = 0; i < iterations; i++) {
        ...
        if (method === "generateCircleProblem") {
          signature = `${qText}|${vars?.S?.x}|${vars?.S?.y}|${vars?.r}`;
        } else if (method === "generateCircleTangentToAxis") {
          signature = `${qText}|${vars?.S?.x}|${vars?.S?.y}|${vars?.axis}`;
        } else if (method === "generateRadiusFromEquation") {
          signature = `${qText}|${vars?.rSq}|${vars?.a}|${vars?.b}`;
        } else if (method === "generateParallelogramVertex") {
          signature = `${qText}|${vars?.D?.x}|${vars?.D?.y}`;
        } else if (method === "generateTriangleAreaCoords") {
          signature = `${qText}|${vars?.area}`;
        }
